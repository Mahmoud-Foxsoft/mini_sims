...
const extractEstimatedAmount = (payload) => {
    ...
    if (estimated === null || estimated === undefined)
        return { estimatePayload, estimated };
    const numeric = Number(estimated);
    return {
        estimatePayload,
        estimated: Number.isFinite(numeric) ? numeric : estimated,
    };
};
...
const createPayment = async () => {
    ...
    if (!paidAmount.value) {
        toast.add({
            severity: "warn",
            summary: "Estimate required",
            detail: "Get an estimate before creating the payment.",
            life: 3000,
        });
        return;
    }
...
<template>
    ...
    >
        <div class="flex flex-col gap-4">
            ...
            <div
                class="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700 dark:border-amber-700 dark:bg-amber-900 dark:text-amber-50"
            >
                It will cost 5% more.
            </div>
            <div class="flex flex-col gap-2">
                ...
                <InputText
                    :modelValue="paidAmount ?? ''"
                    readonly
                    placeholder="Estimate will appear here"
                    class="w-full"
                />
                <small v-if="estimateLoading" class="text-gray-500"
                    >Estimating...</small
                >
                <small v-else-if="estimateError" class="text-red-500">{{
                    estimateError
                }}</small>
            </div>
            <Button
        ...
        >
            ...
            <div class="flex items-center gap-2 mt-3">
                <Button
                    ...
                    @click="copyAddress"
                />
                <span v-if="receipt.pay_amount" class="text-sm text-gray-600"
